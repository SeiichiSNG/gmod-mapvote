...
RTV.TotalVotes = 0
-- wait time in seconds (how long a player has to wait before voting when the map changes)
RTV.Wait = 60
...
function RTV.AddVote(ply)
	if RTV.CanVote(ply) then
		...
		PrintMessage(
			HUD_PRINTTALK,
			ply:Nick() .. " has voted to Rock the Vote. (" .. RTV.TotalVotes .. "/" .. math.Round(#player.GetAll() * 0.66) .. ")"
		)
